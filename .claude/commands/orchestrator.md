...
### 1.1 Analyze Sprint and Define Workstreams (if not done)

**Your exclusive responsibility**: You are the ONLY one who defines workstreams and groups tasks into workstreams. This is a strategic decision that requires understanding task dependencies and parallelization opportunities.
...
```
This will parse existing workstreams and create the sprint configuration.
...
Edit the sprint file to add workstreams section manually, then re-run `pnpm sprint:analyze`.
...
- Name workstreams descriptively (e.g., `ui-components`, `backend-api`, `infrastructure`)
...
**DO:**
- ✅ Set up sprint (analyze sprint, **define workstreams**, create worktrees if needed)
- ✅ **Define workstreams and assign tasks** (strategic decision - your responsibility)
...
1. **Analyze sprint and define workstreams** (if not done): 
   - If workstreams exist: `pnpm sprint:analyze .claude/backlog/sprint-X-<name>.md`
   - If workstreams missing: `pnpm sprint:analyze .claude/backlog/sprint-X-<name>.md --interactive`
